...
import AddToCartButton, { Product as ProductType } from '@/components/AddToCartButton';
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
      ...
      <div className="container mx-auto px-4 py-12">
        {/* Warning */}
        <div className="bg-orange-500/10 border-2 border-orange-500/50 rounded-xl p-6 mb-8 backdrop-blur flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-orange-200 text-sm mb-2">
              Each order must contain products from <strong>only ONE supplier</strong>. Mixing products from different suppliers will result in extra shipping charges.
            </p>
          ...
          <>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredProducts.map((product) => (
                <div key={product.id} className="bg-slate-800/30 border border-slate-700/50 rounded-xl overflow-hidden hover:border-slate-600 transition backdrop-blur group">
                  <div className="h-64 bg-gradient-to-br from-slate-700/50 to-slate-800/50 relative overflow-hidden">
                    {product.imageUrl ? (
                      ...
                      <div className="absolute inset-0 flex items-center justify-center">
                        <div className="text-6xl font-bold text-slate-600/30">{product.name.charAt(0)}</div>
                      </div>
                    )}

                    <div className={`absolute top-4 left-4 ${getSupplierColor(product.supplier?.displayName)} text-white px-3 py-1 rounded-full text-xs font-bold`}>
                      {product.supplier?.displayName || 'Unknown'}
                    </div>

                    {!product.inStock && <div className="absolute top-4 right-4 bg-red-500/90 text-white px-3 py-1 rounded-full text-sm font-semibold">Out of Stock</div>}
                    {product.inStock && <div className="absolute top-14 right-4 bg-green-500/90 text-white px-3 py-1 rounded-full text-sm font-semibold">In Stock</div>}
                    {product.stockQuantity > 0 && product.stockQuantity < 10 && <div className="absolute top-24 right-4 bg-orange-500/90 text-white px-3 py-1 rounded-full text-xs font-semibold">Only {product.stockQuantity} left!</div>}
                  </div>

                  <div className="p-6">
                    <div className="text-xs text-slate-500 uppercase tracking-wide mb-2">{product.category}</div>
                    <h3 className="text-xl font-bold text-white mb-2 group-hover:text-slate-300 transition">{product.name}</h3>
                    <p className="text-slate-400 text-sm mb-4 line-clamp-2">{product.description}</p>
                    <div className="flex items-center justify-between mb-4">
                      <div>
                        <div className="text-2xl font-bold text-white">${product.salePrice.toFixed(2)}</div>
                        <div className="text-xs text-slate-400">Per kit (10 vials of {product.dosage})</div>
                      </div>
                    </div>
            ...
            {filteredProducts.length === 0 && (
              <div className="text-center py-16 bg-slate-800/30 border border-slate-700 rounded-xl">
                ...
                <p className="text-slate-500 text-sm mb-4">Try adjusting your search terms</p>
                {searchQuery && (
                  <button
                    onClick={clearSearch}
